...
RETRY_ABBR = ("QFT, SUSY, CFT, AdS/CFT, EFT, RG, SM, BSM, SUGRA, TQFT, "
              "LIGO, GW, BH, GR, QPO, ISCO, FLRW, ADM, TOV, PBH, EHT, SKA, LISA, EGB")
...
def _build_prompt(cat: str) -> str:
    info = CATEGORY_PROMPTS.get(cat, CATEGORY_PROMPTS["hep-th"])
...
def build_category_html(papers: list[dict], cat: str, out_dir: str, date_display: str, api_date: str):
    ...
    primary = [p for p in papers if p.get("PrimaryCat", "") == cat]
...
def main():
    ...
    parser.add_argument("--cats", nargs="+", default=["hep-th"])
